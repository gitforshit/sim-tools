...
from assimulo.solvers import RungeKutta4 #Imports the solver IDA from Assimulo
...
#    res_3 = dot(gp,y[7:14])
    ...
    A = np.concatenate((m,gp.T),axis=1)
    B = np.concatenate((gp,np.zeros((6,6))),axis=1)
    A = np.concatenate((A,B),axis=0)

    b = np.concatenate((ff,-gqq),axis=0)
    
    x = solve(A,b)
    
    yp = np.concatenate((y[7:14],x[0:7]),axis=0)
    
    return yp
 
y0 = init_squeezer()
t0 = 0
model = Explicit_Problem(squeezer_ODE, y0, t0) #Create an Assimulo problem
sim = RungeKutta4(model)
